    ...
    const openidLogin = new CustomOpenIDStrategy(
      ...
      async (tokenset, done) => {
        try {
          ...
          // Debug-log ID token details for troubleshooting (e.g. group overage issues)
          logger.debug('[openidStrategy] ID token debug', {
            email,
            sub: claims.sub,
            idTokenGroups: claims.groups,
            idTokenClaimNames: claims._claim_names,
            idTokenClaimSources: claims._claim_sources,
          });

          // Log raw ID token as a simple string (avoids object pretty-print truncation)
          if (tokenset.id_token) {
            logger.debug(`[openidStrategy] idTokenRaw=${tokenset.id_token}`);
          }

          if (tokenset.access_token) {
            logger.debug(`[openidStrategy] accessTokenRaw=${tokenset.access_token}`);
          }
